...
const Header = () => {
    ...
    const sidebarClass = isExpanded ? "translate-x-0" : "hidden -translate-x-[20rem]";
    ...
    return (
        <header className="absolute w-[20rem] h-full z-[9999]">
            <div className={`relative shadow-xl shadow-black w-full min-h-[800px] h-full bg-custom-blue transition-all duration-100 ease-in-out ${sidebarClass}`}>
            ...
            <button onClick={toggleSidebar}
                className={`shadow-lg border-2 border-l-0 border-gray-400 bg-[#707070] absolute top-1/2 transform -translate-y-1/2 transition duration-100 ease-in-out ${isExpanded ? "left-[20rem]" : "left-0"}`}>
                {arrowIcon}
            </button>
        </header>
    )
