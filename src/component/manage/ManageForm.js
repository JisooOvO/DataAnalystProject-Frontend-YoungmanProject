 ...
 // eslint-disable-next-line 
  ...
  return (
    <div className="border bg-gray-200 w-full h-fit p-4 sm:p-5 rounded-xl shadow-md sm:text-base text-[70%]">
      <div className="w-full text-center my-5 text-gray-700">
      ...
      <div className="w-[98%] h-[36rem] bg-white relative border">
        <div className="w-full h-fit flex">
          <div style={{width : `calc(100% - 12px)`}} className="grid grid-cols-6 z-20">
            {
              ["거래일","업체명","물품","수량","단가","가격"]
              .map((item,idx)=>
                <div key={`key${idx}`} className="whitespace-nowrap h-16 flex justify-center border border-t-0 border-l-0 items-center text-white font-bold bg-custom-blue">{item}</div>
              )
            }
          </div>
